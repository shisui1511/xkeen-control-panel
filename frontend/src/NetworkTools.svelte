...
<div class="container">
  ...
  {#if publicIP}
    <div class="card mb-3" style="padding: 12px 18px; display: flex; align-items: center; gap: 8px;">
  ...
  <div class="nt-grid mb-3">
    ...
    <div class="nt-card">
      <h3 style="display:flex;align-items:center;gap:8px;">
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="9"/><path d="M3 12h18"/></svg>
        {$t('net.tab_ping')}
      </h3>
      ...
      {#if showSettings.ping}
        <div class="extra-settings mb-2" transition:slide={{ duration: 180 }}>
          ...
          <input id="ping-count" type="number" class="input input-sm" bind:value={count} min="1" max="20" />
        </div>
      {/if}
      <div style="display:flex;gap:8px;margin-top:auto;">
        <button class="btn btn-primary" style="flex:1;" on:click={() => runTool('ping')} disabled={loading || !host}>
          {loading && activeTool === 'ping' ? $t('net.running') : $t('net.run')}
        </button>
        <button class="btn btn-secondary" on:click={() => toggleSettings('ping')} title="Настройки">⋯</button>
      </div>
    </div>
    ...
    <div class="nt-card">
      ...
      {#if showSettings.traceroute}
        <div class="extra-settings mb-2" transition:slide={{ duration: 180 }}>
          ...
          <input id="trace-hops" type="number" class="input input-sm" bind:value={maxHops} min="1" max="30" />
        </div>
      {/if}
      <div style="display:flex;gap:8px;margin-top:auto;">
        <button class="btn btn-primary" style="flex:1;" on:click={() => runTool('traceroute')} disabled={loading || !host}>
          {loading && activeTool === 'traceroute' ? $t('net.running') : $t('net.run')}
        </button>
        <button class="btn btn-secondary" on:click={() => toggleSettings('traceroute')} title="Настройки">⋯</button>
      </div>
    </div>
    ...
    <div class="nt-card">
      <h3 style="display:flex;align-items:center;gap:8px;">
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 4h16v16H4zM4 12h16M12 4v16"/></svg>
        {$t('net.tab_dns')}
      </h3>
      <div class="form-group" style="margin-bottom:10px;">
      ...
      {/if}
      <div style="display:flex;gap:8px;margin-top:auto;">
        <button class="btn btn-primary" style="flex:1;" on:click={() => runTool('dns')} disabled={loading || !host}>
          {loading && activeTool === 'dns' ? $t('net.running') : $t('net.run')}
        </button>
        <button class="btn btn-secondary" on:click={() => toggleSettings('dns')} title="Настройки">⋯</button>
      </div>
    </div>
    ...
    <div class="nt-card">
      ...
      {#if showSettings.http}
        <div class="extra-settings mb-2" transition:slide={{ duration: 180 }}>
          ...
          <input id="http-timeout" type="number" class="input input-sm" bind:value={timeout} min="1" max="60" />
        </div>
      {/if}
      <div style="display:flex;gap:8px;margin-top:auto;">
        <button class="btn btn-primary" style="flex:1;" on:click={() => runTool('http')} disabled={loading || !url}>
          {loading && activeTool === 'http' ? $t('net.running') : $t('net.run')}
        </button>
        <button class="btn btn-secondary" on:click={() => toggleSettings('http')} title="Настройки">⋯</button>
      </div>
    </div>
  </div>
  ...
  {#if result}
    <div class="card card-tight">
      <h2 class="card-title" style="display:flex;justify-content:space-between;align-items:center;">
        <span>{$t('net.result')} — {activeTool.toUpperCase()} {activeTool === 'http' ? url : host}</span>
        <span class="badge" class:badge-success={result.success} class:badge-danger={!result.success}>
