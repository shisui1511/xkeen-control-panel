  ...
  function handleKeydown(event: KeyboardEvent) {
    ...
    if (event.key === 'Tab') {
      ...
      if (event.shiftKey) {
        ...
        if (active === last)  { first.focus(); event.preventDefault(); }
      }
    }
  }
</script>
...
<style>
  ...
  .modal-container {
    ...
    box-shadow: 0 30px 60px -16px rgba(0,0,0,.7), 0 0 0 1px rgba(255,255,255,.02) inset;
  ...
  .modal-close-btn:hover { background: rgba(255,255,255,.05); color: var(--fg-primary); }
  .modal-content {
    padding: 20px;
    overflow-y: auto;
    color: var(--fg-primary);
