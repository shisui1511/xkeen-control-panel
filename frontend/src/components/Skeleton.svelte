...
<style>
  .skeleton {
    background:
      linear-gradient(90deg, rgba(255,255,255,0) 0%, rgba(41,194,240,.08) 50%, rgba(255,255,255,0) 100%),
      var(--bg-elevated);
  ...
  .skeleton-text-line { height: 14px; width: 100%; margin-bottom: 8px; }
  .skeleton-card      { height: 100px; width: 100%; border-radius: var(--radius-md); }
  .skeleton-circle    { width: 40px; height: 40px; border-radius: 50%; }
  .skeleton-rect      { width: 100%; height: 50px; }
  ...
  @media (prefers-reduced-motion: reduce) {
    .skeleton { animation: none !important; background-image: none !important; opacity: .7; }
  }
</style>
