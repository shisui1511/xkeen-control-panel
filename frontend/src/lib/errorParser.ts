export function parseValidationError(rawError: string, lang: 'ru' | 'en'): string {
  ...
  if (
    cleaned.includes('geosite:') &&
    (cleaned.includes('not found') || cleaned.includes('unknown'))
  ) {
    const match =
      cleaned.match(/geosite:\s*([A-Za-z0-9_-]+)/i) ||
      cleaned.match(/([A-Za-z0-9_-]+)\s+not found/i);
  ...
  if (
    cleaned.includes('geoip:') &&
    (cleaned.includes('not found') || cleaned.includes('unknown'))
  ) {
    const match =
      cleaned.match(/geoip:\s*([A-Za-z0-9_-]+)/i) || cleaned.match(/([A-Za-z0-9_-]+)\s+not found/i);
    const tag = match ? match[1] : 'unknown';
    return lang === 'ru'
  ...
  if (cleaned.includes('unknown proxy type')) {
    return lang === 'ru' ? 'Ошибка: Неизвестный тип прокси-сервера.' : 'Error: Unknown proxy type.';
  }
  if (
    cleaned.includes('invalid JSON') ||
    cleaned.includes('json: cannot unmarshal') ||
    cleaned.includes('invalid character')
  ) {
    return lang === 'ru'
