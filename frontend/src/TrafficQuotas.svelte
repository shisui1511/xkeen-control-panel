    ...
    <div class="card mb-2">
      <div class="flex-between mb-2">
        ...
        <button class="btn btn-secondary" on:click={clearAlerts}
          >{$t('trafficquotas.clear_alerts')}</button
        >
      </div>
      {#each alerts as a}
        <div
            ...
            {#each stats.proxies as p}
              <div class="proxy-row">
                ...
                <span class="proxy-traffic"
                  >↑ {formatBytes(p.upload_bytes)} ↓ {formatBytes(p.download_bytes)}</span
                >
              </div>
            {/each}
          </div>
  ...
  <div class="card mb-2">
    <div class="flex-between mb-2">
      ...
      <button class="btn btn-primary" on:click={startCreate}
        >+ {$t('trafficquotas.add_quota')}</button
      >
    </div>
          ...
          <div class="quota-item">
            <div class="quota-main">
              ...
              <div class="quota-details">
                ...
                >
                <span class="detail"
                  >{formatBytes(q.current_bytes)} / {formatBytes(q.limit_bytes)}</span
                >
              </div>
            ...
            <div class="quota-actions">
              <button
                class="btn-icon"
                on:click={() => resetQuota(q.id)}
                title={$t('trafficquotas.reset')}>↺</button
              >
              <button class="btn-icon" on:click={() => startEdit(q)} title={$t('app.edit')}
                ><Icon name="edit" size={14} /></button
              >
              <button class="btn-icon" on:click={() => deleteQuota(q.id)} title={$t('app.delete')}
